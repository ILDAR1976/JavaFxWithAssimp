...
public class WavefrontObjDemo {
    ...
    void loadModel() {
        ...
        File file = new File("./models/animated_cube.dae");
    ...
    void createProgram() throws IOException {
        ...
        int vertexShader = createShader("shaders/bulldozer.vs",
                GL_VERTEX_SHADER_ARB);
        int fragmentShader = createShader("shaders/bulldozer.fs",
    ...
    public long getContext() throws IOException {
        try {
            init();
        } catch (Throwable t) {
            model.free();
